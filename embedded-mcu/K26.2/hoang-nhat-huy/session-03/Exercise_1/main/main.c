...
#define DEBOUNCE_MS      (25U)   // muc tin hieu phai on dinh bao nhieu ms moi tinh la that
#define DOUBLE_CLICK_MS  (350U)  // khoang cach toi da giua 2 lan click de tinh la double click
#define LONG_PRESS_MS    (800U)  // giu bao lau thi bat dau tu dong tang
...
#define GPIO_ENABLE_W1TC_REG  (GPIO_BASE + 0x0028UL)  // tat che do output
#define GPIO_IN_REG           (GPIO_BASE + 0x003CUL)  //doc muc dien ap tren chan

//moi chan co 1 thanh ghi rieng dieu khien GPIO matrix, cach nhau 4 byte
...
//dia chi thanh ghi IO_MUX cua tung chan (moi chan 1 thanh ghi rieng)
...
//vi tri bit trong thanh ghi IO_MUX (giong nhau cho moi chan)
...
//doc/ghi 1 thanh ghi 32-bit qua con tro volatile
#define REG32(dia_chi) (*(volatile uint32_t *)(dia_chi))

//cau hinh chan

//cau hinh 1 chan thanh ngo ra GPIO thuong, muc thap ban dau
static void cau_hinh_chan_ra(uint32_t chan)
{
...
//cau hinh chan nut nhan: ngo vao va bat pull-up noi
...
static uint32_t trang_thai_da_loc = 1U;
...
static void xu_ly_cu_chi(void)
{
  ...
  //click dang cho ma khong co click thu 2 toi kip thi chot lai la +1
