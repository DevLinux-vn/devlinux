...
void print_memory_map(void)
{
    ...
    if (NULL == p_heap_var)
    {
        ...
        exit(1); // Exit if malloc fails to avoid dereferencing NULL pointer.
    }
    *p_heap_var = 20; // Initialize the heap variable.
    ...
    uintptr_t addr_rodata = (uintptr_t)&global_const;     // Address of the global constant (rodata section).
