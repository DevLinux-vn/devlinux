...
# Khi user chạy bash, bash hiện tại (parent bash) sẽ tạo ra một tiến trình mới gọi là child shell (độc lâp với parent shell). Khi child shell được tạo ra nó sẽ nhân một bản sao của các vvariables11 environment từ parent shell. Các variables chưa được export sẽ không được sao chép. Khi child shell được tạo ra, child shell và parent shell hoạt động như hai tiến trình độc lập (mỗi tiến trình có vùng nhớ riêng). Vì là bản sao nên khi child shell thay đổi giá trị của các variables, parent shell sẽ không bị ảnh hưởng